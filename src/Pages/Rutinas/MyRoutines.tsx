...
// SOLUCIÓN AL ERROR DE HOME: Definimos explícitamente el tipo de las props
export const MyRoutines = ({ isEmbedded = false }: { isEmbedded?: boolean }) => {
  
  // Ahora el hook devuelve todo lo necesario
  const { 
  ...
  return (
    <div className={AppStyles.pageContainer + " min-h-screen bg-gray-900"}>
      ...
      {isEmbedded && (
         <div className="absolute inset-0 z-0 opacity-80 pointer-events-none" style={{ backgroundImage: `url(${fondoGym})`, backgroundSize: 'cover' }} />
      )}

      <div className={`${AppStyles.contentContainer} ${isEmbedded ? 'pt-24 px-4' : 'pt-24 px-4'} relative z-10`}>
        <div className="w-full max-w-6xl space-y-6 mx-auto">
          ...
          <div className="mb-10 text-center md:text-left">
              ...
              {isEmbedded && <p className="text-gray-400 text-sm mt-1 animate-pulse">Desliza para ver tu Plan 👉</p>}
          </div>
            ...
            <div className={MyRoutinesStyles.grid}>
              {rutinas.map((rutina) => (
                ...
                >
                  ...
                  {/* BOTÓN DE DESCARGA (Solo App Móvil) */}
                  {isNative && (
                      <button 
        ...
        <div className={AppStyles.modalOverlay} onClick={closeModal}>
           <div className={`${AppStyles.modalContent} max-w-4xl`} onClick={(e) => e.stopPropagation()}>
              ...
              <div className="p-4 border-t border-white/10 bg-gray-800/50 flex justify-end">
                <Button onClick={closeModal} className={AppStyles.btnSecondary + " w-auto px-6 py-2"}>Cerrar</Button>
              </div>
           </div>
        </div>
      )}
