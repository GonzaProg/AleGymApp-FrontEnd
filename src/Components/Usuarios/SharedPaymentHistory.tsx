...
import { CalendarDays, Clock, ShoppingBag, ArrowLeft, Filter, Receipt, X, Download, CheckCircle } from "lucide-react";
import { type AlumnoDTO } from "../../API/Usuarios/UsuarioApi";
import { CustomSelect } from "../UI/CustomSelect";

interface SharedPaymentHistoryProps {
    user: AlumnoDTO;
    onBack: () => void;
...
export const SharedPaymentHistory = ({ user, onBack, showPlanSummary = false, isEmbedded = false }: SharedPaymentHistoryProps) => {
    ...
    // Lógica para showPlanSummary
    const planesActivos = user.userPlans?.filter((p: any) => p.activo);
    const ultimoPlanActivo = planesActivos?.length ? planesActivos[0] : null;
    const ultimoPlanInactivo = user.userPlans
    ...
    return (
        <div className={`w-full ${isEmbedded ? '' : 'max-w-5xl mx-auto mt-14'} animate-fade-in text-white relative`}>
            ...
            <div className="bg-[#1e1628]/95 backdrop-blur-xl border border-white/10 p-6 rounded-2xl shadow-2xl relative z-10 w-full mb-8">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h2 className="text-2xl font-bold text-white tracking-tight">
                                {isEmbedded ? "Historial de Pagos" : "Historial Completo"}
                            </h2>
                ...
                {/* Resumen del Último Plan (Opcional) */}
